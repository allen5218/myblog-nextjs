...
在 `scripts/site-font-plan.mjs` 的 `placeNewAssignments` 之後、`buildFontPlan` 之前加入:

```js
const REBALANCE_RESTARTS = 512
...
function shuffledOrder(groups, random) {
  const order = [...groups]
...
// 依「出現在哪一組文件」把字元分組。拆開同一組只會讓該組所有文件多碰一個桶,
// 而碰桶數正是預算的約束瓶頸,所以同組字元永遠同桶。
function signatureGroups({ corpus, core, committedAssignments }) {
  ...
  for (const codePoint of sortedCodePoints(candidates)) {
    const pages = [...(corpus.occurrences.get(codePoint) ?? [])]
      .map((name) => documentIndex.get(name))
      .filter((index) => index !== undefined)
      .sort((left, right) => left - right)
    const signature = pages.join(',')
    const group = bySignature.get(signature) ?? { signature, pages, codePoints: [] }
  ...
  return { groups, documentCount: documents.length }
}

function greedyPlacement({ order, documentCount, bucketCount, maxBucketsPerDocument }) {
  ...
  for (const group of order) {
    ...
    for (let bucket = 0; bucket < bucketCount; bucket += 1) {
      const newlyTouched = group.pages.filter((page) => !documentBuckets[page].has(bucket)).length
      const violates = group.pages.some(
        (page) =>
          !documentBuckets[page].has(bucket) &&
          documentBuckets[page].size >= maxBucketsPerDocument
      )
      const key = [violates ? 1 : 0, newlyTouched, bucketSizes[bucket], bucket]
      if (!best || compareKeys(key, best.key) < 0) best = { bucket, key }
    }
    placement.set(group.signature, best.bucket)
    bucketSizes[best.bucket] += group.codePoints.length
    for (const page of group.pages) documentBuckets[page].add(best.bucket)
  }
  return placement
}

function scorePlacement({ groups, placement, documentCount, bucketCount }) {
  const bucketSizes = Array(bucketCount).fill(0)
  const documentBuckets = Array.from({ length: documentCount }, () => new Set())
  for (const group of groups) {
    const bucket = placement.get(group.signature)
    bucketSizes[bucket] += group.codePoints.length
    for (const page of group.pages) documentBuckets[page].add(bucket)
  }
  let worstBuckets = 0
  let worstCodePoints = 0
  for (const touched of documentBuckets) {
    worstBuckets = Math.max(worstBuckets, touched.size)
    worstCodePoints = Math.max(
      worstCodePoints,
      [...touched].reduce((sum, bucket) => sum + bucketSizes[bucket], 0)
    )
  }
  return [worstBuckets, worstCodePoints]
}
...
export function rebalanceAssignments({
  ...
  const { groups, documentCount } = signatureGroups({ corpus, core, committedAssignments })
  let best = null
  for (let restart = 0; restart < REBALANCE_RESTARTS; restart += 1) {
    // restart 0 用穩定排序本身;其餘用固定 seed 的確定性擾動。
    const order =
      restart === 0 ? groups : shuffledOrder(groups, seededRandom(REBALANCE_SEED + restart))
    const placement = greedyPlacement({
      order,
      documentCount,
      bucketCount,
      maxBucketsPerDocument,
    })
    const key = scorePlacement({ groups, placement, documentCount, bucketCount })
    // 嚴格小於才取代 → 平手時保留較早的 restart,結果與跑幾次無關。
    if (!best || compareKeys(key, best.key) < 0) best = { key, placement }
  }

  if (best.key[0] > maxBucketsPerDocument) {
    throw new Error(
      `Chiron rebalance could not keep every document within ${maxBucketsPerDocument} buckets (best: ${best.key[0]})`
    )
  }

  const assignments = new Map()
  for (const group of groups) {
    const bucket = best.placement.get(group.signature)
    for (const codePoint of group.codePoints) assignments.set(codePoint, bucket)
  }
  return assignments
}
```
