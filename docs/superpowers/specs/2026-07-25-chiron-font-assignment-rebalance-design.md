...
1. **依文件簽名分組。** 把所有 non-core 的 supported code point 依「它出現在哪些文件」
   分組(目前 845 cp → 196 組)。同組字元永遠同桶:拆開一組只會讓該組所有文件多碰一個桶,
   而碰桶數正是這裡的約束瓶頸(拆組雖能微調各桶大小,但 bytes 有充足餘裕,不值得用約束換)。
2. **確定性排序。** 組別依 `(size desc, signature asc, 最小 code point asc)` 排序。
3. **種子化搜尋。** 以常數 seed 的 LCG 產生固定次數的重啟(`REBALANCE_RESTARTS`)。每次重啟
