...
- Precache **只含 `/offline/`**:`createSerwistRoute` 明確設 `globPatterns: []`,避免預設規則
  在首次安裝就下載整個 `.next/static` 與 `public/`(包含非當前頁面的圖片、JS/CSS、OG
  專用字型與非當前頁面的 Chiron 網站字型 bucket)。其他資源繼續由 `defaultCache` 在實際
  使用時按需快取;不要移除這個空陣列,也不要把網站字型加回 precache。
- 這項取捨只保證 `/offline/` HTML 預快取,其 CSS/JS chunk 仍依賴 runtime cache。若使用者
  首次造訪後立即離線,後備頁可能能顯示但暫時沒有樣式;這是避免安裝時預抓全站資產的已知
  極端情況。
...
- `yarn update:site-font` 會先 fresh build Contentlayer model,再更新目前 buckets,不會讀取
  ...
  `check` job 會以 `origin/main` assignment map 為歷史基準,完整檢查 glyph、cmap、variable
  axis、exact-byte budgets 與既有 assignment 未被換桶;初次 rollout 可以沒有 base map,
  之後移除或移動既有 assignment 都會讓 CI 失敗。Vercel 仍跑 schema/hash/CSS/corpus/budget
  靜態驗證,只能在缺少工具時跳過動態檢查。
- Corpus 掃描若回報 `Unknown Unicode category for U+...`,代表遇到尚未經刻意決策的字元
