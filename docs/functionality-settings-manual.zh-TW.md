...
- 子集架構是一個 committed 單調 core 加五個可重用 supplemental buckets。
  ...
  一起 commit,不可手改。
- `yarn update:site-font` 會更新目前 buckets,不縮減 core;只有明確的 `--rebuild-core`
...
| `yarn update:site-font` | 重新產生 committed Chiron 網站字型 buckets;只有刻意單調擴張 core 時才加 `--rebuild-core`(§9)                                                            |
