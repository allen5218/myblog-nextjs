...
- **GFM**(表格、任務清單、刪除線)加 GitHub 風格警示引言。

### GitHub 風格警示引言

文章可用 `remark-github-blockquote-alert` 把重要補充渲染成醒目的提示框。第一行必須是
大寫的 `[!TYPE]`,後續每一行都保留 `>`:

```markdown
> [!NOTE]
> 這是讀者即使快速瀏覽也值得知道的補充資訊。
>
> 同一個提示框可以包含多個段落。
```
...
| 類型 | 適合內容 |
|------|----------|
| `NOTE` | 補充背景、定義或容易略過的資訊 |
| `TIP` | 能更快、更省力完成事情的實用技巧 |
| `IMPORTANT` | 完成目標前必須掌握的關鍵資訊 |
| `WARNING` | 需要立即注意,否則可能出錯的事項 |
| `CAUTION` | 可能造成資料遺失、安全問題或其它負面後果的高風險操作 |
...
提示框應用來突出真正需要掃讀辨識的內容,不要把一般段落或連續多段正文都包成 alert;
同一段只選一種最符合後果嚴重度的類型。
...
- Precache 只含 `/offline/` 與其**呈現依賴**:`createSerwistRoute` 不沿用預設 glob(那會在
  ...
  supplemental 字型 buckets)維持按需快取 — `/_next/static` 的 JS 由 `app/sw.ts`
  置前的 CacheFirst route(沿用 `next-static-js-assets` cache 名)處理,其餘仍走
  `defaultCache`;不要把 glob 改回預設,也不要把 supplemental 字型加進 precache。
- 這保證離線後備頁與線上**一致**(樣式、字型、hero 圖都來自 precache),即使使用者首次
  ...
  暖入上述置前的 `next-static-js-assets` CacheFirst 快取,離線時照樣完成 hydration
