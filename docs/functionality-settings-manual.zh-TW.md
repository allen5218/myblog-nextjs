...
- **圖表**:` ```mermaid ` fence 於**建置期**渲成一組淺/深雙 SVG — **沒有客戶端 Mermaid
  ...
  (`.github/workflows/mermaid-check.yml`,job 名 `mermaid`,非必過檢查)在 push/PR 時重跑
  `yarn mermaid:render --check`,標記過期的快取。
...
- `yarn build` / `yarn check:og-font` / `yarn update:og-font`需要本機安裝 HarfBuzz CLI
  ...
  而不是失敗(見 §6)— 因為 Vercel 上沒有 HarfBuzz;GitHub Action `og-font-check`
  會在 push/PR 時補跑這道檢查(見 §6)。
- `yarn mermaid:render` 需要本機安裝 Playwright Chromium
  ...
  `mermaid-check` 會在 push/PR 時補跑 `--check`(見 §2),但不是必過檢查,快取過期不會
  擋合併或部署。
