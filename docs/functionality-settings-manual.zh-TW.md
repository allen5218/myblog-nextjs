...
- 這保證離線後備頁與線上**視覺一致**(樣式、字型、hero 圖都來自 precache),即使使用者
  首次造訪後立即離線。JS 刻意不預快取:後備頁是 server-rendered,外觀不依賴 hydration。
  Playwright(`tests/playwright/serwist-precache.spec.ts`)驗證 precache 內容與離線樣式。
