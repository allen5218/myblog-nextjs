...
- **Chiron 站字型的產物必須整組一起 commit**(`public/static/fonts/chiron/*.woff2`、manifest、
  `css/chiron-font.generated.css`),**不要手改 generated CSS/manifest**。既有 code point
  不得被普通更新重排、core 不得縮減;產物過期時跑 `yarn update:site-font`(只有刻意擴張
  core 才加 `--rebuild-core`)。pipeline 細節、CI/Vercel 差異與所需 CLI 見
