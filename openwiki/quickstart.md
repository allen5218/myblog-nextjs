...
- **Mermaid image dimensions** — [`lib/rehype-mermaid.mjs`](../lib/rehype-mermaid.mjs), [`css/tailwind.css`](../css/tailwind.css), [`scripts/mermaid-shared.mjs`](../scripts/mermaid-shared.mjs): `normalizeSvg` already puts intrinsic `width`/`height` on each committed SVG root; there is no Mermaid manifest. The emitted theme `<img>` elements still lack dimensions, so CSS reserves no box before load (CLS); both are lazy, including the hidden variant, which can briefly blank a slow theme switch.
- **Mermaid render diagnostics and deterministic churn** — [`scripts/mermaid-render.mjs`](../scripts/mermaid-render.mjs): one invalid diagram aborts the fail-loud render before any cache output is written; retain that no-corruption guarantee if adding diagnostics or recovery. Re-rendering a Gantt also changes its off-viewBox `today` marker’s x-coordinate by date even when its source is unchanged, causing harmless `public/mermaid/` churn.
