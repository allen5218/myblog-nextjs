...
- **Mermaid theme-switch loading gap** — [`lib/rehype-mermaid.mjs`](../lib/rehype-mermaid.mjs): emitted theme images now copy rounded dimensions from their committed SVG roots, so they reserve a box before loading; no Mermaid manifest is needed. Both variants remain `loading="lazy"`, including the `display:none` variant, so a slow theme switch can briefly show a correctly sized empty box.
- **Mermaid render diagnostics** — [`scripts/mermaid-render.mjs`](../scripts/mermaid-render.mjs): rendering is intentionally transactional and fail-loud—an invalid diagram or unusable SVG root aborts the run before cache writes. Keep that no-corruption guarantee if adding diagnostics or recovery. Static Gantts must retain `todayMarker off`; [`tests/unit/mermaid-gantt-contract.test.ts`](../tests/unit/mermaid-gantt-contract.test.ts) enforces it across `data/blog/` so render-time “today” cannot freeze or cause date-driven artifact churn.
