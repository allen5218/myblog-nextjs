...
The assignment history is deliberately stable for ordinary updates: new code points are placed deterministically without moving existing assignments, and core must not shrink. A budget failure can justify the explicit one-off `yarn update:site-font --rebalance` path: it deterministically rewrites supplemental placement, increments `font-data/chiron/assignment-epoch.txt`, and invalidates the affected font-cache URLs. Do not advance that epoch by hand. The CI check compares assignment history only when the current epoch equals its base epoch; an advance deliberately skips that comparison and reports a warning, while core history remains monotonic. The checker also validates source/manifest/CSS consistency, glyph coverage, shaping, and per-page byte/request budgets. Never hand-edit the generated CSS or manifest; regenerate and commit the complete output set.
