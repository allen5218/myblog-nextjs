...
The visible site uses a committed Chiron core plus five supplemental Unicode buckets. Authoritative inputs are `font-data/chiron/core-codepoints.txt`, `font-data/chiron/source.json`, `font-data/chiron/supplemental-assignments.json`, and `font-data/chiron/assignment-epoch.txt`. Generated outputs are `public/static/fonts/chiron/*.woff2`, its manifest, and `css/chiron-font.generated.css`.

The assignment history is deliberately stable for ordinary updates: new code points are placed deterministically without moving existing assignments, and core must not shrink. The static corpus seeds printable ASCII, shared UI copy, dictionaries, and site metadata alongside article Markdown. Add a character to the fixed seed only when it is rendered through the Chiron font chain and exists in the source font: each seed enters core and full validation shapes that subset. KaTeX output is deliberately excluded—its HTML and MathML use `KaTeX_Main` and `math`, respectively, so DOM presence cannot trigger a Chiron request. A budget failure can justify the explicit one-off `yarn update:site-font --rebalance` path: it deterministically reassigns every non-core code point and increments `font-data/chiron/assignment-epoch.txt`. Because artifact filenames contain content hashes, that rewrites all five supplemental URLs, so returning readers re-download the complete supplemental set. Do not advance the epoch by hand. The CI check always enforces monotonic core history when a base core is supplied. It compares assignment history while epochs match; an advance reports that comparison as skipped, but must also match the deterministic rebalance output exactly. If `--base-epoch` is omitted, the checker treats it as the committed epoch and still compares history, rather than failing open. If a rebalance introduces fixed-UI seed characters, it must also use `--rebuild-core`, because every fixed seed must be in core. The checker also validates source/manifest/CSS consistency, glyph coverage, shaping, and per-page byte/request budgets. Never hand-edit generated CSS or manifest; regenerate and commit the complete input/output set.
...
When changing Mermaid version, theme variables, or `normalizeSvg` in a way that changes rendered SVG output:
...
A validation-only renderer gate that writes identical SVG bytes does not require a `CACHE_VERSION` bump: bumping would regenerate every cached artifact on a different machine or date and manufacture the spurious diff that versioning avoids. `yarn mermaid:render --check` is intentionally structural only—hash/file coverage, no byte-for-byte redraw—because browser text measurement differs across platforms. The GitHub Mermaid workflow is advisory, not a required merge check. A genuine cache miss (`ENOENT`) degrades to the original code block and is reported by `--check`; a present cache file whose root has no usable `width`/`height` fails loudly during content compilation. Both variants are read before that decision, so a missing one cannot conceal a corrupt sibling. The renderer likewise refuses to write an unusable root, and writes nothing until every diagram and variant has rendered successfully. Producer and consumer deliberately share `parseSvgRootDimensions` in [`scripts/mermaid-shared.mjs`](../../scripts/mermaid-shared.mjs), avoiding incompatible acceptance rules. The plugin copies those root dimensions, rounded to HTML integers, to each emitted theme `<img>` so the browser reserves its box before the SVG arrives; no Mermaid manifest is involved. Common causes of a code-block fallback are forgotten render artifacts, stale `.contentlayer` output, an invalid `mermaid:title` fence (only exact `mermaid` is recognized), or a fence outside `data/blog/`.
